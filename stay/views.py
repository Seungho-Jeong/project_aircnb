...
class StayDetailView(View):
    ...
    def get(self, request, stay_id):
        try:
            ...
            detail = [
                {
                    ...
                    "rental_type": stay.stay_type.name,
                    ...
                    "review": {
                        "avr_score": (
                                             stay.review_set.aggregate(Avg("cleanliness_star"))[
                                                 "cleanliness_star__avg"] +
                                             stay.review_set.aggregate(Avg("communication_star"))[
                                                 "communication_star__avg"] +
            ...
            return JsonResponse({"MESSAGE": "ERROR_IS_{}".format(e)}, status=400)
...
class StayListView(View):
    ...
    def get(self, request):
        try:
            ...
            # 렌탈타입, 편의시설를 필터링할 때
            elif rental_type_id and amenity_id:
                stays = Stay.objects.annotate(
                    sum_of_beds=Sum("room__beds"),
                    sum_of_rooms=Count("room__name")
                ).filter(
            ...
            # 렌탈타입, 집 종류를 필터링할 때
            elif rental_type_id and subcategory_id:
                stays = Stay.objects.annotate(
                    sum_of_beds=Sum("room__beds"),
                    sum_of_rooms=Count("room__name")
                ).filter(
            ...
            # 편의시설, 집 종류를 필터링할 때
            elif amenity_id and subcategory_id:
                stays = Stay.objects.annotate(
                    sum_of_beds=Sum("room__beds"),
                    sum_of_rooms=Count("room__name")
                ).filter(
            ...
            detail = [{
                ...
                "discount_price": int(stay.discount_price),
                "subcategory": stay.subcategory.name,
                "rental_types": stay.stay_type.name,
                "capacity": stay.capacity,
                "beds": stay.room_set.aggregate(Sum("beds"))["beds__sum"],
                "rooms": stay.room_set.all().count(),
                ...
                "review": {"avr_score": (stay.review_set.aggregate(Avg("cleanliness_star"))["cleanliness_star__avg"] +
                                         stay.review_set.aggregate(Avg("communication_star"))[
                                             "communication_star__avg"] +
                                         stay.review_set.aggregate(Avg("checkin_star"))["checkin_star__avg"] +
                                         stay.review_set.aggregate(Avg("accuracy_star"))["accuracy_star__avg"] +
                                         stay.review_set.aggregate(Avg("location_star"))["location_star__avg"] +
                                         stay.review_set.aggregate(Avg("value_star"))["value_star__avg"]) / 6,
                           "reviews": stay.review_set.count(), },
                "bathrooms": stay.bathrooms,
